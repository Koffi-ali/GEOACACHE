...
function Remove_cache(){
    ...
    const handleSubmit = (event) =>{
        ...
        fetch('http://localhost:3000/remove_geocaches', {
        method: 'POST',
        headers: {
...
return (
  <>
    ...
    >
    <div className='stuck'>
      <h2 style={{textAlign:'center'}}>Supprimer la cache</h2>
      <form onSubmit={handleSubmit}>
        <label>
          Longitude:
          <input type="text" value={longitude} defaultValue={longitude} onChange={(event) => setLongitude(event.target.value)} readOnly />
        </label>
        <br />
        <label>
          Latitude:
          <input type="text" value={latitude} defaultValue={latitude} onChange={(event) => setLatitude(event.target.value)} readOnly/>
        </label>
        <br />
        <label>
          Difficulté:
          <input type="text" value={difficulty} defaultValue={difficulty} onChange={(event) => setDifficulty(event.target.value)} readOnly />
        </label>
        <br />
        <label>
          Description:
          <textarea value={description} defaultValue={description} onChange={(event) => setDescription(event.target.value)} readOnly/>
        </label>
        <br />
